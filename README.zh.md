...
最简单——用 [`npx skills`](https://github.com/vercel-labs/skills)：

```bash
npx skills add https://github.com/zxl-lxz/commentmd
```

或者手动 clone 后把 skill 目录 symlink 到 Agent 的 skills 目录下：
...
```bash
python3 skills/commentmd/scripts/serve.py path/to/方案.md
```
...
```bash
python3 skills/commentmd/scripts/serve.py path/to/方案.md --static /tmp/review.html
```
